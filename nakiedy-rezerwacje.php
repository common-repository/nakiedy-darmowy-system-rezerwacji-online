...
/*
  ...
  Plugin URI: https://www.nakiedy.pl
  Description: Wtyczka do rezerwacji wizyt oprarta na systemie rezerwacji online Nakiedy. Aby z niej korzystać wejdź na www.nakiedy.pl i <a href="https://app.nakiedy.pl/zarejestruj.html" target="_blank">zarejestruj konto</a>. Instalacja: 1) Kliknij odnośnik "Włącz" po lewej od tego opisu, 2) <a href="https://app.nakiedy.pl/zarejestruj.html" target="_blank">Zarejestruj się, by zdobyć klucz API do Nakiedy</a>, oraz 3) Przejdź na stronę ustawień nowej pozycji w menu "Rezerwacje" i wprowadź swój klucz API. Teraz możesz na dowolnej podstronie umieścić system rezerwacji.
  Version: 1.2.1
  Author: System rezerwacji Nakiedy
  Author URI: https://www.nakiedy.pl
  License: GPLv2+
...
class Nakiedy_Rezerwacje{
    ...
    function __construct() {
        ...
        add_filter( 'content_edit_pre', array( $this,'wpn_change_wysiwyg'),10,2);
    ...
    public function wpn_save_data( $nakiedy_token ) {
        ...
        $api = new RestClient(array(
            'base_url' => "https://api.nakiedy.pl/v1/api",
            'format' => "json",
    ...
    function wpn_save_meta_box_data( $post_id ) {
        ...
        if ($my_data==0){
            ...
            if (strpos($content,'id="nakiedy-tag"')){
                $content=$this->replaceTag($content,'nakiedy-tag','<img','/>','');
    ...
    function wpn_content_filter( $content ) {
        ...
        if ( is_singular('page') ){
            ...
            if (!empty($value)){
                if (get_option('nakiedy_url') && strpos($content,'id="nakiedy-tag"')){
                    $iframe='<iframe src="https://'.get_option('nakiedy_url').'/em/?h='.((int)get_option('nakiedy_height')-270).'" style="'.'width: '.get_option('nakiedy_width').'px; height: '.get_option('nakiedy_height').'px; border:0;max-width:100%;"'.' id="nakiedyWidget"></iframe>';
                    $content=$this->replaceTag($content,'nakiedy-tag','<img','/>',$iframe);
                }
                elseif (strpos($content,'id="nakiedy-tag"')){
                    $img_file=plugins_url('images/nakiedy-spacer-configure.jpg', __FILE__);
                    $img='<img src="'.$img_file.'" alt="System rezerwacji Nakiedy"/>';
                    $content=$this->replaceTag($content,'nakiedy-tag','<img','/>',$img);
                }
            }
        }
        // Returns the content.
    ...
    function wpn_change_wysiwyg($content,$post_id) {
        global $post;
    ...
    function replaceTag($str,$id,$start_tag,$end_tag,$replace)
    {
        ...
        if (!empty($allposts) && is_array($allposts))
            foreach ($allposts as $post){
                if (strpos($post->post_content,'id="nakiedy-tag"')){
                    $post->post_content=$this->replaceTag($post->post_content,'nakiedy-tag','<img','/>','');
                    wp_update_post($post);
                }
