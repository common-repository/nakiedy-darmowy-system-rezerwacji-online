  ...
  if ( isset( $_POST["save_code"]) and isset($_POST["nakiedy_token"]) ) {
    ...
    if($res===true){
        ...
        exit();
          ...
          $update_message = '<div id="setting-error-settings_updated" class="updated settings-error below-h2"><p><strong>Błędny klucz API lub brak możliwości połączenia ('.$res.'). W razie problemów prosimy o kontakt (kontakt@nakiedy.pl)</strong></p></div>';
      }
  }
...
<div class="wrap">
  ...
  <form action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>&noheader=true" method="post" name="settings_form" id="settings_form">
    <table width="1004" class="form-table">
      ...
      <tr>
          <th colspan="2">
              <span style="font-size: 15px; line-height: 2.3;">Nie masz jeszcze konta <span style="color:#E46500;">Nakiedy</span>?? </span><a href="https://www.nakiedy.pl/zarejestruj.html" target="_blank" class="button media-button button-large">Wypóbuj za darmo</a>
          </th>
      </tr>
      </tbody>
    </table>
  ...
  <h3>Podgląd</h3>
  <iframe src="https://<?php echo get_option('nakiedy_url')?>/em?h=<?php echo $nakiedy_height-270;?>" id="nakiedyWidget" style="width: <?php echo $nakiedy_width;?>px; height: <?php echo $nakiedy_height;?>px; border:0;"></iframe>
    <?php endif;?>
<?php    }?>
</div>
</div>